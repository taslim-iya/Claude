...
export default function Profile() {
  ...
  const Field = ({ label, value, onChange }: { label: string; value: string; onChange: (v:string)=>void }) => (
    <div>
      <label className="block text-[11px] font-semibold uppercase tracking-wider mb-1.5"
        style={{ color:'var(--text-2)' }}>{label}</label>
  ...
  const Toggle = ({ value, onChange }: { value: boolean; onChange: ()=>void }) => (
    <button onClick={onChange}
      ...
      style={{ background:value?'#5b6ef9':'var(--border-2)' }}>
  ...
  return (
    <div className="p-6 max-w-3xl animate-fade-in">
      <div className="mb-6">
        ...
        <p className="text-sm mt-0.5" style={{ color:'var(--text-2)' }}>Manage your personal account settings.</p>
      </div>
      ...
      <div className="rounded-xl p-6 mb-4" style={{ background:'var(--surface)', border:'1px solid var(--border)' }}>
        ...
        <div className="flex items-center gap-5 mb-6">
          ...
          <div>
            <p className="font-semibold" style={{ color:"var(--text)" }}>{name}</p>
            <p className="text-sm" style={{ color:'var(--text-2)' }}>{title}</p>
            <p className="text-xs mt-0.5" style={{ color:'var(--text-3)' }}>Admin · Acme Sales Co</p>
          </div>
      ...
      <div className="rounded-xl p-6 mb-4" style={{ background:'var(--surface)', border:'1px solid var(--border)' }}>
        <h3 className="text-sm font-semibold  mb-4 flex items-center gap-2" style={{ color:"var(--text)" }}><Mail size={14}/>Email Sender Settings</h3>
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-[11px] font-semibold uppercase tracking-wider mb-1.5"
              style={{ color:'var(--text-2)' }}>Email Signature</label>
      ...
      {/* Notifications */}
      <div className="rounded-xl p-6 mb-4" style={{ background:'var(--surface)', border:'1px solid var(--border)' }}>
        <h3 className="text-sm font-semibold  mb-4 flex items-center gap-2" style={{ color:"var(--text)" }}><Bell size={14}/>Notifications</h3>
        <div className="space-y-4">
          {[
        ...
        <div className="space-y-2">
          <button className="w-full text-left flex items-center justify-between p-3 rounded-xl transition-colors"
            ...
            <div>
              ...
              <p className="text-xs" style={{ color:'var(--text-3)' }}>Last changed 90 days ago</p>
            </div>
          ...
          <button className="w-full text-left flex items-center justify-between p-3 rounded-xl transition-colors"
            ...
            <div>
              ...
              <p className="text-xs" style={{ color:'var(--text-3)' }}>Add extra security to your account</p>
            </div>
