...
import { Download, Trash2, AlertCircle, UserX, Shield, ToggleLeft, ToggleRight, Brain, Mail } from 'lucide-react';

type Tab = 'bounces' | 'unsubscribes';
type BounceFilter = 'all' | 'hard' | 'soft';

type Bounce = { id:string; email:string; name:string; company:string; type:string; reason:string; date:string; campaign:string };
type Unsub  = { id:string; email:string; name:string; reason:string; date:string; campaign:string; sentiment:string; sentColor:string };
...
export default function BounceManager() {
  ...
  const hardRate = total > 0 ? ((hardCount / total) * 100).toFixed(1) : '0.0';
  const softRate = total > 0 ? ((softCount / total) * 100).toFixed(1) : '0.0';
                  ...
                  <tr>
                    <td colSpan={8}>
                      <div className="flex flex-col items-center justify-center py-12 text-center">
                        ...
                        <p className="text-sm font-medium" style={{ color:'var(--text-2)' }}>No bounces recorded</p>
                        <p className="text-xs mt-1" style={{ color:'var(--text-3)' }}>Bounced emails from your campaigns will appear here</p>
                      </div>
                    </td>
                  </tr>
              ...
              <tbody>
                {UNSUBS.length === 0 ? (
                  <tr>
                    <td colSpan={6}>
                      <div className="flex flex-col items-center justify-center py-12 text-center">
                        <UserX size={28} className="mb-2" style={{ color:'var(--border-2)' }} />
                        <p className="text-sm font-medium" style={{ color:'var(--text-2)' }}>No unsubscribes yet</p>
                        <p className="text-xs mt-1" style={{ color:'var(--text-3)' }}>Opt-out requests from your campaigns will appear here</p>
                      </div>
                    </td>
                  </tr>
                ) : UNSUBS.map(u => (
